...
import sto.builder.PositionIndexer;
...
//	}
//	
	public static MessageObjectContainer createObjectContainerWithResult(String message) throws NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException, InstantiationException, ClassNotFoundException, NoSuchMethodException, InvocationTargetException{
	...
	public static MessageObjectContainer createObjectContainer(String message) throws NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException, InstantiationException, ClassNotFoundException, InvocationTargetException, IntrospectionException, NoSuchMethodException{
		MessageObjectContainer container = new MessageObjectContainer();
		PositionIndexer positionIndexer = new PositionIndexer();
		
		TestHeader header = StringToObjectBuilder.buildObject(TestHeader.class, testString,positionIndexer);
		container.header = header;
		
		int count = header.getCount();
		for(int i = 0; i < count; i++){
			TestDetail detail = StringToObjectBuilder.buildObject(TestDetail.class, testString,positionIndexer);
			container.getDetails().add(detail);
		}
